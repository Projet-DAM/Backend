...
export class PaymentsController {
  ...
  async createPaymentIntent(@Body() createPaymentIntentDto: CreatePaymentIntentDto) {
    ...
    if (childId && offerId) {
      ...
      subscriptionId = undefined; // Will be created later after payment
      ...
      if (phoneNumber) {
        try {
          ...
          // Don't block payment flow if phone update fails
        }
      }
    }
  ...
  async completePayment(
    ...
    try {
      ...
      console.log('📧 Nom:', parent.nom);
      console.log('📧 Prénom:', parent.prenom);
      console.log('📧 Offre:', offer.name);
      console.log('📧 Montant:', `${(paymentIntent.amount / 100).toFixed(2)} €`);
      console.log('📧 Date début:', startDate.toLocaleDateString('fr-FR'));
